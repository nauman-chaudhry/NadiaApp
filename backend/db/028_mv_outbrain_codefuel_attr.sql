-- Migration 028: attribute Codefuel-Outbrain revenue per campaign
--
-- FIX (this migration): Codefuel-Outbrain revenue was surfaced as one daily
-- aggregate orphan row (cost showed per campaign, revenue lumped). Now it is
-- attributed per campaign via outbrain_ads (gd, r) -> campaign (migration 027),
-- matching how IA-Outbrain already attributes. Verified 100% of Codefuel-Outbrain
-- revenue maps. Only genuinely unmapped (gd, r) fall to the orphan row.
-- Carries forward the granularity de-dup (`ps` CTE) from 026.
--
