-- Migration 021: attribute ALL Image Advantage revenue to its campaign
--
-- NEW FIX (this migration): IA rows in partner_stats_hourly carry campaign_ext_id
--   PREFIXED with the source platform, e.g. 'tb:49546219', while campaigns.external_id
--   stores the bare id '49546219'. The prior IA join (c.external_id = p.campaign_ext_id)
--   therefore matched only the unprefixed rows — 6,060 of 12,798 (47%) — dumping the
--   other 53% ($1,295 total IA -> only ~half attributed) into the orphan bucket.
--   Verified: stripping the 'tb:' prefix makes 12,798/12,798 (100%) attribute.
--   FIX = regexp_replace(p.campaign_ext_id, '^tb:', '') in ia_attributed + ia_orphan.
--
-- Everything else identical to migration 020 (same-row cost+revenue join).
--
...
ia_attributed AS (
  ...
  -- Strip 'tb:' prefix so IA campaign_ext_id matches campaigns.external_id.
...
ia_orphan AS (
  ...
  WHERE NOT EXISTS (
      SELECT 1 FROM campaigns cx WHERE cx.external_id = regexp_replace(p.campaign_ext_id, '^tb:', '')
    )
