-- Migration 026: granularity de-duplication (prefer hourly over daily)
--
-- FIX (this migration): partner_stats_hourly holds BOTH 'daily' and 'hourly'
-- rows. On days where both exist for the same partner (e.g. the daily->hourly
-- switchover window), the MV summed both and DOUBLE-COUNTED revenue. We add a
-- `ps` CTE that keeps every hourly row and keeps a daily row only when NO hourly
-- row exists for that (partner, calendar day). All revenue CTEs read from `ps`.
-- Everything else is identical to migration 025.
--
...
ob_ia_rev AS (
  SELECT regexp_replace(p.campaign_ext_id, '^ob:', '') AS campaign_id,
...
-- Cost + IA revenue per (campaign, day).
ob_rev_by_camp_day AS (
  ...
  FROM ob_ia_rev GROUP BY campaign_id, day_utc
),
ob_campaign_days AS (
...
-- Codefuel-Outbrain revenue (no campaign map yet) -> daily orphan.
ob_cf_orphan AS (
