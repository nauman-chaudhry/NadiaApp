-- Migration 034: resolve IA Taboola user_tag via item id, + connect Lead Gen 3
--
-- Problem found while integrating the flux affiliates (ssm.flux.*.tb): the IA
-- user_tag first segment is NOT always a Taboola campaign id. For the flux
-- campaigns it is the Taboola ITEM id — their tracking template is
-- ut={item_id}.{site_id}.{site} (8-digit ids are campaigns, 10-digit '428…'/
-- '4286…' ids are items). The old ia_attributed only matched campaign ids, so
-- ALL item-id-keyed IA revenue (flux + a large slice of n2s) fell into the
-- single "(Unattributed revenue — Image Advantage)" orphan bucket and never
-- showed as individual campaigns.
--
-- Fix: resolve the user_tag seg1 (stored as campaign_ext_id 'tb:<seg1>') via
-- campaigns.external_id FIRST, then fall back to ads.external_id (item) → its
-- campaign. This mirrors the Outbrain link-id resolution added in migration 030.
-- It attributes flux revenue to its real campaign+account, and also recovers the
-- previously-orphaned n2s item-id revenue. Total IA revenue is unchanged — it
-- just moves from the orphan lump onto named campaign rows.
--
-- Also: tag "Lead Gen Affiliates 3 - SC" with image_advantage so it carries the
-- IA badge and appears under the Image Advantage source filter (the flux
-- campaigns live under both Lead Gen 2 — already IA — and Lead Gen 3).
--
-- Requires: the IA-referenced items must be present in `ads` (synced via
-- listItems in syncTaboolaMetadata phase 3). Unresolved seg1 values still fall
-- to the orphan bucket — no revenue is lost.
--
-- Carries forward the x-daily granularity + de-dup exemption from migration 033.

-- Connect Lead Gen Affiliates 3 to Image Advantage (flux campaigns live here).
UPDATE ad_accounts
   SET client_partner_id = (SELECT id FROM client_partners WHERE code = 'image_advantage')
 WHERE external_id = 'tdg-sevenspheremedia4433-leadgenaffiliates-3-sc';
