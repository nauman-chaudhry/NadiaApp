-- Add campaign_status to joined_stats_hourly so the UI status filter works.
-- Rebuilds on top of the migration-002 campaign-level design (not the ad-level
-- FULL OUTER JOIN approach that was here before, which failed for Taboola rows
-- because ad_id IS NULL and the EXISTS subquery never matched).
