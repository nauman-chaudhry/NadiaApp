-- =============================================================================
-- Migration 002: Support campaign-level Taboola stats + redesigned joined view
-- =============================================================================
--
-- Background:
--   Taboola's reporting API provides campaign-level hourly stats only — no
--   item / site / country / device breakdown at hourly granularity.
--   Stats are stored with ad_id = NULL. The original unique constraint
--   UNIQUE(ad_account_id, ad_id, hour_utc, country, device, site) does not
--   enforce uniqueness when ad_id IS NULL (Postgres NULL != NULL). This
--   migration adds a partial unique index that covers that case.
--
--   The materialized view is redesigned to join at campaign level:
--   ad_stats_hourly.campaign_id → campaigns → ads → partner_stats_hourly
--   (via gd_param/r_param). Codefuel revenue is aggregated per campaign per
--   hour/day before joining with Taboola cost.
-- =============================================================================

-- Partial unique index for campaign-level rows (ad_id IS NULL).
-- The ON CONFLICT clause in the sync upsert targets this index.
CREATE UNIQUE INDEX IF NOT EXISTS idx_ad_stats_campaign_level
  ON ad_stats_hourly (ad_account_id, campaign_id, hour_utc, country, device, site)
  WHERE ad_id IS NULL;

-- =============================================================================
-- Redesigned materialized view
-- =============================================================================
...
-- Campaign metadata (static lookup, one row per campaign)
campaign_meta AS (
  SELECT
    c.id                 AS campaign_id,
    ...
    acc.external_id      AS ad_account_external_id,
...
-- Codefuel hourly revenue summed per campaign per hour.
-- Multiple items in the same campaign may have different asset_gid/channel
-- values; we aggregate them all so one campaign-hour row represents total
-- revenue for that campaign that hour.
codefuel_hourly AS (
  SELECT
    a.campaign_id,
    p.hour_utc,
...
-- Codefuel daily revenue per campaign (for historical data outside the 14-day hourly window).
-- hour_utc on daily rows is 00:00 of the calendar day.
codefuel_daily AS (
  SELECT
    a.campaign_id,
...
-- Taboola stats aggregated to daily level (for joining with daily Codefuel).
taboola_by_day AS (
  SELECT
    campaign_id,
    ad_account_id,
...
-- ── Part 1: Hourly rows (last ~14 days where both sources have hourly data) ──
SELECT
...
-- ── Part 2: Daily rows (historical backfill — Codefuel daily × Taboola daily) ──
-- Only shows days where Codefuel has 'daily'-granularity rows (pre-14-day window).
-- Taboola daily spend is the sum of all hourly rows for that calendar day.
SELECT
...
-- Recreate indexes on the new view
