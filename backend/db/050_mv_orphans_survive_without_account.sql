-- 050_mv_orphans_survive_without_account.sql
--
-- Supersedes 047. ONE change: the two orphan-account laterals (ia_orphan, ddc_orphan)
-- become LEFT JOIN LATERAL. With an inner join, a deployment that has NO account
-- tagged for the partner (e.g. Joe before onboarding, or a partner whose accounts
-- were never tagged) silently DROPPED every unmatched IA/DDC revenue row from the
-- view — revenue vanished instead of appearing as unattributed (review
-- 2026-09-24, finding 4). Now the row is kept with ad_account_id NULL, exactly like
-- the Codefuel orphan branch. When a tagged account exists nothing changes.
-- api/server.ts already resolves the same account with a scalar subquery that
-- yields NULL in that case, so Campaign == Ads holds either way.
--
-- 047 header follows (kept for the change history):
...
ia_orphan AS (
  ...
  LEFT JOIN LATERAL (SELECT id AS ad_account_id FROM ad_accounts WHERE client_partner_id = cp.id
